...
clear 
close all
%% sequenza pulita con maschera, vedi relazione 
...
i_s = 84;           % indice finale parte a galleria spenta
i_transitorio = 48; % fotogrammi da non considerare

stat = uint8(mean(PAR.T(:,:,1:i_s),3));           % figure(10); imshow(CS_fast(stat));           
din  = uint8(PAR.T(:,:,(i_s+i_transitorio):end)); % figure(11); imshow(CS_fast(uint8(mean(din,3))));
din_1 = uint8(din - repmat(stat,1,1,size(din,3)));% figure(12); imshow(CS_fast(uint8(mean(din_1,3))));


seq = uint8(din_1) - repmat(uint8(mean(din_1,3)),1,1,size(din_1,3));
...
KC = 0;  % somma luminosit� campionamento
KS = 0;  % quantità da sommare alla luminosit� subcampionamento
...
if FLAG_cycle == 1
    for k = k_start:k_end
        ...
        if FLAG_save == 1
            saveas(hFig,...
            strcat('./24fps_mask/figure',num2str(k),'_fps_',num2str(fps),'.png'));
        else
            if k == 30
                pause(2)
                disp('...pulisco plot...')
...
fprintf('Sthrual sez maestra ... = %6.3f +- %2.3f (imax = %d)\n',sthrual_maestro,toll_sthrual,imax);
